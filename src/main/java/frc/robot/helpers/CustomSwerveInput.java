 ...
 * speeds. Intended to easily create an
...
public class CustomSwerveInput implements Supplier<ChassisSpeeds> {
     ...
     * {@link ChassisSpeeds} from a driver controller.
     *
     ...
     */
    public CustomSwerveInput(SwerveDrive drive, DoubleSupplier x, DoubleSupplier y, DoubleSupplier rot) {
        this(drive, x, y);
     ...
     */
    public CustomSwerveInput(SwerveDrive drive, DoubleSupplier x, DoubleSupplier y, DoubleSupplier headingX,
    ...
    public static CustomSwerveInput of(SwerveDrive drive, DoubleSupplier x, DoubleSupplier y) {
        return new CustomSwerveInput(drive, x, y);
    }

    /**
     * Copy the {@link CustomSwerveInput} object.
     *
     * @return Clone of current {@link CustomSwerveInput}
     */
    public CustomSwerveInput copy() {
        CustomSwerveInput newStream = new CustomSwerveInput(swerveDrive, controllerTranslationX,
     ...
     * @return self
     */
    public CustomSwerveInput driveToPose(Supplier<Pose2d> pose, ProfiledPIDController xPIDController,
     ...
     * @return self
     */
    public CustomSwerveInput driveToPose(Supplier<Pose2d> pose, ProfiledPIDController translation,
     ...
     * @return self
     */
    public CustomSwerveInput headingOffset(Rotation2d angle) {
     ...
     * @return self
     */
    public CustomSwerveInput withHeading(Supplier<Rotation2d> heading) {
     ...
     * @return this
     */
    public CustomSwerveInput aim(Pose2d aimTarget) {
        ...
        if (driveToPoseEnabled.isPresent() && driveToPoseEnabled.get().getAsBoolean()) {
            ...
            if (aimTarget.isPresent()) {
                ...
                DriverStation.reportError(
                        "Attempting to enter AIM mode without target, please use CustomSwerveInput.aim() to select a target first!",
                        false);
            }
            ...
            if (headingSupplier.isPresent() || (controllerHeadingX.isPresent() && controllerHeadingY.isPresent())) {
                ...
                DriverStation.reportError(
                        "Attempting to enter HEADING mode without heading information, please use CustomSwerveInput.withHeading or CustomSwerveInput.withControllerHeadingAxis!",
                        false);
            }
        } else if (controllerOmega.isEmpty()) {
            DriverStation.reportError(
                    "Attempting to enter ANGULAR_VELOCITY mode without a rotation axis, please use CustomSwerveInput.withControllerRotationAxis to add angular velocity axis!",
                    false);
            return SwerveInputMode.TRANSLATION_ONLY;
        }
     ...
     *         {@link CustomSwerveInput#translationCube} is present.
     */
    ...
    public ChassisSpeeds get() {
        ...
        switch (newMode) {
            ...
            case AIM -> {
                ...
                omegaRadiansPerSecond = swerveController.headingCalculate(currentHeading.getRadians(),
                        target.getRadians());
                speeds = new ChassisSpeeds(vxMetersPerSecond, vyMetersPerSecond, omegaRadiansPerSecond);
                break;
            }
