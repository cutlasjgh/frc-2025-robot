...
public class NavXSwerve extends SwerveIMU
{
  ...
  public NavXSwerve(NavXComType port)
  {
    ...
    try
    {
      ...
      imu = new AHRS(port,AHRS.NavXUpdateRate.k50Hz); //JC
      //factoryDefault();  // original call, lets at least zeroyaw jc
      //imu.reset(); //jc wanted to add, reported to be slow, so just do yawzero fo rnow
      imu.zeroYaw();//jc added
      offset = imu.getRotation3d();//jc added
